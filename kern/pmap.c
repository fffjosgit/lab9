...
//
static inline void
page_rmv(struct PageInfo *ppage)
{
    struct PageInfo *p = page_free_list;
    if(!ppage) {
        return;
    }
    if(page_free_list == ppage) {
        page_free_list = ppage->pp_link;
        ppage->pp_link = 0;
        return;
    }
    while(p->pp_link && (p->pp_link != ppage)) {
        p = p->pp_link;
    }
    if(!p->pp_link) {
        return;
    }
    p->pp_link = ppage->pp_link;
    ppage->pp_link = 0;    
}
...
page_init(void)
{
    ...
    //kernel 
    for(i = (unsigned int)KERNBASE; i < (unsigned int)boot_alloc(0); i += PGSIZE) {
        ppage = pa2page(PADDR((void *)i));
        page_rmv(ppage);
    }
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pde & PTE_P) {
	    ...
	    if((pp = page_alloc(ALLOC_ZERO))) {
	        pgdir[PDX(la)] = page2pa(pp) | PTE_W | PTE_U | PTE_P;
	        pp->pp_ref++;
...
check_page_installed_pgdir(void)
{
	...
	page_insert(kern_pgdir, pp2, 0x0, PTE_W);
	assert(*(int *)va == 0x02020202);
	assert(pp2->pp_ref == 1);
	assert(pp1->pp_ref == 0);
	*(uint32_t *)va = 0x03030303;
	assert(*(uint32_t *)page2kva(pp2) == 0x03030303);
